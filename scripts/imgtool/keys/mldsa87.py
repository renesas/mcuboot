"""
MLDSA87 key management using FIPS 204 specification
"""

# SPDX-License-Identifier: Apache-2.0

import os
from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives import serialization

from .general import KeyClass

# dilithium-py is an optional dependency: it is only needed to actually
# generate, sign or verify with ML-DSA keys. Unconditional import would make
# imgtool unusable for RSA/ECDSA/Ed25519 users who have not installed it.
try:
    from dilithium_py.ml_dsa import ML_DSA_87
except ImportError:
    ML_DSA_87 = None


class Mldsa87UsageError(Exception):
    pass


def _ml_dsa_87():
    """Return the ML-DSA-87 implementation, or explain how to install it."""
    if ML_DSA_87 is None:
        raise Mldsa87UsageError(
            "ML-DSA-87 support requires the 'dilithium-py' package. "
            "Install it with 'pip install dilithium-py' (see "
            "scripts/requirements.txt), or use a non-ML-DSA key type.")
    return ML_DSA_87


class Mldsa87Public(KeyClass):
    def __init__(self, public_key_bytes):
        """Initialize with raw public key bytes from FIPS 204 ML-DSA-87"""
        if not isinstance(public_key_bytes, bytes):
            raise Mldsa87UsageError("Public key must be bytes")
        if len(public_key_bytes) != 2592:  # FIPS 204 ML-DSA-87 public key size
            raise Mldsa87UsageError(f"Invalid public key size: {len(public_key_bytes)} bytes, expected 2592")
        self.public_key_bytes = public_key_bytes

    def shortname(self):
        return "mldsa87"

    def _unsupported(self, name):
        raise Mldsa87UsageError(f"Operation {name} requires private key")

    def _get_public(self):
        return self.public_key_bytes

    def get_public_bytes(self):
        """Return raw public key bytes"""
        return self.public_key_bytes

    def get_private_bytes(self, minimal, format):
        self._unsupported('get_private_bytes')

    def export_private(self, path, passwd=None):
        self._unsupported('export_private')

    def export_public(self, path):
        """Write the public key to the given file as raw bytes."""
        try:
            with open(path, 'wb') as f:
                f.write(self.public_key_bytes)
        except OSError as e:
            raise Mldsa87UsageError(f"Failed to write public key to {path}: {e}")

    def sig_type(self):
        return "MLDSA87"

    def sig_tlv(self):
        return "MLDSA87"

    def sig_len(self):
        return 4627  # FIPS 204 ML-DSA-87 signature length

    def verify_digest(self, signature, digest):
        """Verify that signature is valid for given digest using FIPS 204 ML-DSA"""
        ml_dsa = _ml_dsa_87()
        try:
            # Use FIPS 204 ML-DSA-87 for verification
            return ml_dsa.verify(self.public_key_bytes, digest, signature)
        except Exception:
            return False


class Mldsa87(Mldsa87Public):
    """
    Wrapper around an MLDSA87 private key using FIPS 204 specification.

    Provides methods for key generation, signing, and exporting both
    private and public keys in various formats for post-quantum cryptography.
    Uses ML-DSA-87 parameter set for higher security than MLDSA44.
    """

    def __init__(self, private_key_bytes, public_key_bytes):
        """Initialize with raw private and public key bytes from FIPS 204 ML-DSA-87"""
        if not isinstance(private_key_bytes, bytes):
            raise Mldsa87UsageError("Private key must be bytes")
        if len(private_key_bytes) != 4896:  # FIPS 204 ML-DSA-87 private key size
            raise Mldsa87UsageError(f"Invalid private key size: {len(private_key_bytes)} bytes, expected 4896")

        super().__init__(public_key_bytes)
        self.private_key_bytes = private_key_bytes

    @staticmethod
    def generate():
        """Generate a new MLDSA87 key pair using FIPS 204 ML-DSA specification"""
        # Use FIPS 204 ML-DSA-87 for key generation
        public_key, private_key = _ml_dsa_87().keygen()

        return Mldsa87(private_key, public_key)

    def _get_public(self):
        return self.public_key_bytes

    def get_private_bytes(self, minimal, format):
        """Return raw private key bytes"""
        if format == 'raw':
            return self.private_key_bytes
        else:
            raise Mldsa87UsageError(f"get_private_bytes not supported with format {format} for {self.shortname()} keys")

    def export_private(self, path, passwd=None):
        """
        Write the private key to the given file with both private and public key data.
        Format: 4 bytes length + private key + public key
        Total size: 4 + 4896 + 2592 = 7492 bytes
        """
        if passwd is not None:
            raise Mldsa87UsageError("Password protection not supported for raw dilithium keys")
        
        try:
            with open(path, 'wb') as f:
                import struct
                # Write header: private_key_length (4 bytes little-endian)
                f.write(struct.pack('<I', len(self.private_key_bytes)))
                # Write private key bytes
                f.write(self.private_key_bytes)
                # Write public key bytes
                f.write(self.public_key_bytes)
        except OSError as e:
            raise Mldsa87UsageError(f"Failed to write private key to {path}: {e}")

    def sign_digest(self, digest):
        """Return the actual signature using FIPS 204 ML-DSA"""
        # Use FIPS 204 ML-DSA-87 for signing
        signature = _ml_dsa_87().sign(self.private_key_bytes, digest)

        return signature





# Debugging Code
        """
# Remove old keys
rm mykey.pem

# Generate new key with enhanced format (usese 16-byte seed from os.random())
python imgtool.py keygen -t mldsa-87 -k mykey.pem

# Check file size (should be 7492 bytes now since both public and private keys are present)
ls -la mykey.pem

# Test public key extraction 
python imgtool.py getpub -k mykey.pem -e raw -o pubkey.bin

# Check public key size (should be 2592 bytes)
ls -la pubkey.bin

# Test signing (4627-byte packed signatures)
python imgtool.py sign -k mykey.pem --version 2.0.0+0 --header-size 0x80 --align 128 --max-align 128 --slot-size 0x8000 --max-sectors 1 --overwrite-only --pad-header input.bin output.bin

# Test verification. Not sure if we should change this to accept public key... that may be not necessary since that happens on the MCU
python imgtool.py verify -k mykey.pem output.bin
"""